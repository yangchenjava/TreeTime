...
@interface YCCircularSlider ()

@property (strong, nonatomic) CALayer *outerCircleLayer;

@end

@implementation YCCircularSlider
...
- (void)drawWithLocation:(CGPoint)location {
    ...
    // 防止循环滑动
    CGFloat progress = angle / (360.f - _cutoutAngle);
    if (fabs(progress - _progress) > 0.8 && progress > _progress) {
        progress = 0.f;
    }
    if (fabs(progress - _progress) > 0.8 && progress < _progress) {
        progress = 1.f;
    }
    if (fabs(progress - _progress) > 0.2) {
        return;
    }
